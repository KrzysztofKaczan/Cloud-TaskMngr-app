...
public class TasksController : ControllerBase
{
    ...
    public async Task<ActionResult<TaskReadDto>> GetById(int id)
    {
        ...
        return Ok(new TaskReadDto { Id = task.Id, Name = task.Name, IsCompleted = task.IsCompleted });
    }       
 
    [HttpPost]
    public async Task<ActionResult<TaskReadDto>> Create(TaskCreateDto taskDto)
    {
        var newTask = new CloudTask { Name = taskDto.Name, IsCompleted = false };
        _context.Tasks.Add(newTask);
        await _context.SaveChangesAsync();

        var readDto = new TaskReadDto { Id = newTask.Id, Name = newTask.Name, IsCompleted = newTask.IsCompleted };
